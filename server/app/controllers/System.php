...
use HoneySens\app\models\ContactService;
...
class System extends RESTResource {
    ...
    static function registerRoutes($app, $em, $beanstalk, $config, $messages) {
        $app->get('/triggerWeeklySummary', function() use ($app, $em, $beanstalk, $config, $messages) {
        ...
        $app->get('/api/system', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new System($em, $beanstalk, $config);
            $systemData = $controller->get();
            echo json_encode($systemData);
        });

        $app->delete('/api/system/events', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new System($em, $beanstalk, $config);
            try {
        ...
        $app->delete('/api/system/db', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new System($em, $beanstalk, $config);
            $messages = array();
            try {
        ...
        $app->put('/api/system/db', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new System($em, $beanstalk, $config);
            $messages = array();
        ...
        $app->post('/api/system/install', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new System($em, $beanstalk, $config);
        ...
        $app->get('/api/system/update', function() use ($app, $em, $beanstalk, $config, $messages) {
        ...
        $app->post('/api/system/update', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new System($em, $beanstalk, $config);
            $controller->update();
            echo json_encode([]);
        });
    ...
    public function get() {
        ...
        $x509 = new \File_X509();
        try {
            $cert = $x509->loadX509(file_get_contents($certfile));
    ...
    function initDBSchema(&$messages, $em, $skipPermissionCheck=false, $forceReset=false) {
        ...
        if($forceReset || !$schemaManager->tablesExist(array('users'))) {
            ...
            // Default admin user
            $admin = new User();
            $admin->setName('admin')->setPassword(sha1('admin'))->setEmail('root@localhost')->setRole($admin::ROLE_ADMIN);
            $em->persist($admin);
            // Default sensor config
            $defaultConfig = new SensorConfig();
            $defaultConfig->setRecon(true)->setKippoHoneypot(true)->setDionaeaHoneypot(true)->setUpdateInterval(20);
    ...
    function update() {
        ...
        $this->getBeanstalkService()->putUpdateJob();
    }
    ...
    private function addLastUpdatesTable($em) {
        ...
        $connection->prepare('INSERT INTO last_updates (table_name, timestamp) VALUES ("configs", NOW()), ("images", NOW()), ("sensors", NOW()), ("users", NOW()), ("divisions", NOW()), ("contacts", NOW()), ("settings", NOW()), ("event_filters", NOW()), ("stats", NOW())')->execute();
    }
}