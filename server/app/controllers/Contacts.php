...
class Contacts extends RESTResource {
    ...
    static function registerRoutes($app, $em, $beanstalk, $config, $messages) {
        $app->get('/api/contacts(/:id)/', function($id = null) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Contacts($em, $beanstalk, $config);
            $criteria = array();
        ...
        $app->delete('/api/contacts/:id', function($id) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Certs($em, $beanstalk, $config);
