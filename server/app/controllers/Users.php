...
class Users extends RESTResource {
    ...
    static function registerRoutes($app, $em, $beanstalk, $config, $messages) {
        $app->get('/api/users(/:id)/', function($id = null) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Users($em, $beanstalk, $config);
            $criteria = array();
        ...
        $app->post('/api/users', function() use ($app, $em, $beanstalk, $config) {
            $controller = new Users($em, $beanstalk, $config);
        ...
        $app->put('/api/users/:id', function($id) use ($app, $em, $beanstalk, $config) {
            $controller = new Users($em, $beanstalk, $config);
            $request = $app->request()->getBody();
            V::json()->check($request);
            $userData = json_decode($request);
        ...
        $app->delete('/api/users/:id', function($id) use ($app, $em, $beanstalk, $config) {
            $controller = new Users($em, $beanstalk, $config);
