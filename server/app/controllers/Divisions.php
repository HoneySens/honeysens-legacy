...
class Divisions extends RESTResource {
    ...
    static function registerRoutes($app, $em, $beanstalk, $config, $messages) {
        $app->get('/api/divisions(/:id)/', function($id = null) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Divisions($em, $beanstalk, $config);
            $criteria = array();
        ...
        $app->post('/api/divisions', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Divisions($em, $beanstalk, $config);
        ...
        $app->put('/api/divisions/:id', function($id) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Divisions($em, $beanstalk, $config);
            $request = $app->request()->getBody();
            V::json()->check($request);
            $divisionData = json_decode($request);
        ...
        $app->delete('/api/divisions/:id', function($id) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Divisions($em, $beanstalk, $config);
