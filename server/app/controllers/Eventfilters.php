...
class Eventfilters extends RESTResource {
    ...
    static function registerRoutes($app, $em, $beanstalk, $config, $messages) {
        $app->get('/api/eventfilters(/:id)/', function($id = null) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Eventfilters($em, $beanstalk, $config);
            $criteria = array();
        ...
        $app->post('/api/eventfilters', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Eventfilters($em, $beanstalk, $config);
        ...
        $app->put('/api/eventfilters/:id', function($id) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Eventfilters($em, $beanstalk, $config);
            $request = $app->request()->getBody();
            V::json()->check($request);
            $filterData = json_decode($request);
        ...
        $app->delete('/api/eventfilters/:id', function($id) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Eventfilters($em, $beanstalk, $config);
