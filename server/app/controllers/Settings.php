...
class Settings extends RESTResource {
    ...
    static function registerRoutes($app, $em, $beanstalk, $config, $messages) {
        $app->get('/api/settings', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Settings($em, $beanstalk, $config);
            $settings = $controller->get();
            echo json_encode($settings);
        });

        $app->put('/api/settings', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Settings($em, $beanstalk, $config);
        ...
        $app->post('/api/settings/testmail', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Settings($em, $beanstalk, $config);
            $request = $app->request()->getBody();
            V::json()->check($request);
