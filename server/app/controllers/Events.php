...
class Events extends RESTResource {
    ...
    static function registerRoutes($app, $em, $beanstalk, $config, $messages) {
        $app->get('/api/events(/:id)/', function($id = null) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Events($em, $beanstalk, $config);
        ...
        $app->post('/api/events', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Events($em, $beanstalk, $config);
        ...
        $app->put('/api/events/:id', function($id) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Events($em, $beanstalk, $config);
            $request = $app->request()->getBody();
            V::json()->check($request);
        ...
        $app->put('/api/events', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Events($em, $beanstalk, $config);
            $request = $app->request()->getBody();
            V::json()->check($request);
            $eventData = json_decode($request, true);
        ...
        $app->delete('/api/events/:id', function($id) use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Events($em, $beanstalk, $config);
        ...
        $app->delete('/api/events', function() use ($app, $em, $beanstalk, $config, $messages) {
            $controller = new Events($em, $beanstalk, $config);
            $request = $app->request()->getBody();
            V::json()->check($request);
        ...
        //$app->get('/api/events/test', function() use ($app, $em, $beanstalk, $config, $messages) {
            //$controller = new Events($em, $beanstalk, $config);
            //$controller->test();
        //});
    }
	...
	public function create($data, ConfigParser $config) {
		...
		$mailService = new ContactService();
		foreach($events as $event) {
